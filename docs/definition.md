...
A `scope` is basically a static model method, which supports chaining.
...
But the `mixin` method could also be used to load the `definition`, `model` and `record` part of a `plugin` (See [Plugins](#plugins)).
...
If you didn't specify a `getUserBy` function, it will search for `context.user.id` (See [Context](./query.md#context)). Otherwise the `creator_id` and `updater_id` will stay null.
