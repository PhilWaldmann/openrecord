  ...
  describe(title + ': Joins', function() {
    ...
    describe('join()', function() {
      ...
      it('join returns the right sql', function() {
        return store.ready(function() {
          ...
          return User.join('posts').toSql(function(sql) {
            sql.should.be.equal(
              'select "users"."id" "f0", "users"."login" "f1", "users"."email" "f2", "users"."created_at" "f3", "posts"."id" "f4", "posts"."user_id" "f5", "posts"."thread_id" "f6", "posts"."message" "f7" from "users" inner join "posts" on "users"."id" = "posts"."user_id"'
            )
          })
        })
      })
      ...
      it('leftJoin returns the right sql', function() {
        return store.ready(function() {
          ...
          return User.leftJoin('posts').toSql(function(sql) {
            sql.should.be.equal(
              'select "users"."id" "f0", "users"."login" "f1", "users"."email" "f2", "users"."created_at" "f3", "posts"."id" "f4", "posts"."user_id" "f5", "posts"."thread_id" "f6", "posts"."message" "f7" from "users" left join "posts" on "users"."id" = "posts"."user_id"'
            )
          })
        })
      })
      ...
      it('rightJoin returns the right sql', function() {
        ...
        return store.ready(function() {
          ...
          return User.rightJoin('posts').toSql(function(sql) {
            sql.should.be.equal(
              'select "users"."id" "f0", "users"."login" "f1", "users"."email" "f2", "users"."created_at" "f3", "posts"."id" "f4", "posts"."user_id" "f5", "posts"."thread_id" "f6", "posts"."message" "f7" from "users" right join "posts" on "users"."id" = "posts"."user_id"'
            )
          })
        })
      })
      ...
      it('join with a belongsTo relation', function() {
        return store.ready(function() {
          ...
          return Post.join('user').toSql(function(sql) {
            sql.should.be.equal(
              'select "posts"."id" "f0", "posts"."user_id" "f1", "posts"."thread_id" "f2", "posts"."message" "f3", "user"."id" "f4", "user"."login" "f5", "user"."email" "f6", "user"."created_at" "f7" from "posts" inner join "users" "user" on "posts"."user_id" = "user"."id"'
            )
          })
        })
      })
      ...
      it('returns the right results on multiple joins', function() {
        return store.ready(function() {
          var User = store.Model('User')
          return User.join('posts', 'threads')
            .order('users.id')
            .exec(function(result) {
              result[0].login.should.be.equal('phil')
              result[0].posts.length.should.be.equal(3)
      ...
      it('returns the right results on nested joins ', function() {
        return store.ready(function() {
          var Thread = store.Model('Thread')
          return Thread.join({ posts: 'user' })
            .order('title')
            .exec(function(result) {
              ...
              result[0]._posts[2]._user.login.should.be.equal('michl')
              result[1].title.should.be.equal('second thread')
              result[1]._posts.length.should.be.equal(1)
              result[1]._posts[0]._user.login.should.be.equal('phil')
            })
        })
      })
      ...
      it('returns the right results on nested joins with the same table twice', function() {
        return store.ready(function() {
          var Thread = store.Model('Thread')
          return Thread.join({ posts: 'user' }, 'user')
            .order('title', 'user.id')
            .exec(function(result) {
              result[0].title.should.be.equal('first thread')
              result[0]._posts.length.should.be.equal(3)
              ...
              result[0]._posts[1]._user.login.should.be.equal('phil')
              result[0]._posts[2]._user.login.should.be.equal('michl')
              result[0]._user.login.should.be.equal('michl')
              result[1].title.should.be.equal('second thread')
              result[1]._posts.length.should.be.equal(1)
              result[1]._posts[0]._user.login.should.be.equal('phil')
              result[1]._user.login.should.be.equal('phil')
            })
        })
      })
      ...
      it('returns the right results on multiple nested joins and nested conditions (attribute = attribute)', function() {
        return store.ready(function() {
          var User = store.Model('User')
          return User.join({ threads: 'posts' })
            .where({ threads: { posts: { id: { $attribute: 'user_id' } } } })
            .order('users.id')
            .exec(function(result) {
              result.length.should.be.equal(1)
              result[0]._threads[0]._posts[0].id.should.be.equal(
                result[0].threads[0].posts[0].user_id
              )
            })
        })
      })
      ...
      it('returns the right results on multiple nested joins and nested conditions (attribute = other_table.attribute)', function() {
        return store.ready(function() {
          ...
          return User.join({ threads: 'posts' })
            .where({
              threads: {
                posts: { id: { $attribute: 'id', $parents: ['threads'] } }
              }
            })
            .order('users.id')
            .exec(function(result) {
      ...
      it('returns the right results on deep nested joins with nested conditions', function() {
        return store.ready(function() {
          var Thread = store.Model('Thread')
          return Thread.join({ posts: { user: 'posts' } }, 'user')
            .where(
              { posts: { user: { login_like: 'phi' } } },
              { title_like: 'first' }
            )
            ...
            .exec(function(result) {
              ...
              result[0]._posts[0]._user.login.should.be.equal('phil')
              result[0]._posts[1]._user.login.should.be.equal('phil')
              result[0]._posts[0]._user._posts.length.should.be.equal(3)
              result[0]._user.login.should.be.equal('michl')
              should.not.exist(result[1])
            })
        })
          ...
          return Thread.join({ posts: { user: 'posts' } }, 'user')
            .where({
              posts: {
                user: {
                  posts: {
                    user_id: { $attribute: 'user_id', relation: 'posts' }
                  }
                }
              }
            })
            ...
            .exec(function(result) {
              ...
              result[0]._posts[0].user_id.should.be.equal(
                result[0]._posts[0]._user._posts[0].user_id
              )
            })
        })
      })
      ...
      it('returns the right results on hasOne relations', function() {
        return store.ready(function() {
          ...
          return User.join('avatar', 'left').exec(function(result) {
            ...
            should.not.exist(result[3]._avatar)
          })
        })
      })
        ...
        return store.ready(function() {
          ...
          return User.leftJoin('unread')
            ...
            .exec(function(result) {
              ...
              result[3]._unread.length.should.be.equal(0)
            })
        })
      })
      ...
      it('returns the right results on nested hasMany through', function() {
        return store.ready(function() {
          var User = store.Model('User')
          return User.leftJoin('unread_threads')
            .order('users.id')
            .exec(function(result) {
              result.length.should.be.equal(4)
              ...
              result[3]._unread_threads.length.should.be.equal(0)
            })
        })
      })
      ...
      it('returns the right results on sub nested hasMany through', function() {
        return store.ready(function() {
          var User = store.Model('User')
          return User.join({ threads: { user: 'unread_threads' } })
            .order('users.id')
            .exec(function(result) {
              result.length.should.be.equal(1)
      ...
      it('returns the right results on sub nested hasMany through with conditions', function() {
        return store.ready(function() {
          ...
          return User.join({ threads: { user: 'unread_threads' } })
            .where({
              threads: { user: { unread_threads: { title_like: 'second' } } }
            })
            .order('users.id')
            .exec(function(result) {
              result.length.should.be.equal(1)
      ...
      it('throws an error on polymorphic join', function() {
        return store
          ...
          .should.be.rejectedWith(Error, {
            message: "Can't join polymorphic relations"
      ...
      it('join returns the right sql (custom join)', function() {
        return store.ready(function() {
          var User = store.Model('User')
          return User.join('JOIN posts ON posts.id = users.id').toSql(function(
            sql
          ) {
            sql.should.be.equal(
              'select * from "users" JOIN posts ON posts.id = users.id'
            )
          })
        })
      })
      ...
      it('join returns the right sql (custom join with args)', function() {
        return store.ready(function() {
          var User = store.Model('User')
          return User.join([
            'JOIN posts ON posts.id = users.id AND posts.message LIKE ?',
            ['%foo%']
          ]).toSql(function(sql) {
            sql.should.be.equal(
              'select * from "users" JOIN posts ON posts.id = users.id AND posts.message LIKE \'%foo%\''
            )
          })
        })
      })
