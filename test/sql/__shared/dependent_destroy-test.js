  ...
  describe(title + ': Destroy dependent', function() {
    ...
    it('destroy hasMany with failing relation destroy', function() {
      return store
        .ready(function() {
          ...
          return Thread.find(1).then(function(thread) {
    ...
    it('destroy hasMany with failing nested relation destroy', function() {
      return store
        .ready(function() {
          ...
          return Thread.find(3).then(function(thread) {
            return thread.destroy()
          })
        })
    ...
    it('destroy belongsTo', function() {
      return store.ready(function() {
        ...
        return Post.find(3).then(function(post) {
          return post.destroy().then(function() {
            return Thread.find(2).then(function(thread) {
    ...
    it('destroy belongsTo with failing relation destroy', function() {
      return store
        .ready(function() {
          ...
          return Post.find(5).then(function(post) {
            return post.destroy()
          })
        })
        .should.be.rejectedWith(Error, { message: 'stop from user' })
