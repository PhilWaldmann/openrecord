  ...
  describe(title + ': Temporary Definition', function() {
    ...
    before(function() {
      ...
      store.Model('User', function() {
        ...
        this.scope('tmpValidation', function() {
          this.temporaryDefinition().validatesFormatOf('login', /phil.*/)
        })

        this.scope('tmpHook', function() {
          this.temporaryDefinition().beforeSave(function() {
            throw new Error('stop')
          })
        })

        this.scope('tmpRelation', function() {
          this.temporaryDefinition().hasMany('threads')
        })
        ...
        this.scope('tmpConversion', function() {
          this.temporaryDefinition().convertOutput(
            'login',
            function(value) {
    ...
    it('does not pollute the model validation definition', function() {
      return store.ready(function() {
        var User = store.Model('User')
        return User.create({ login: 'max' }).then(function(result) {
    ...
    it('does not pollute the model hooks definition', function() {
      return store.ready(function() {
        var User = store.Model('User')
        User.create({ login: 'max' }).then(function(result) {
