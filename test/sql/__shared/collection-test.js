      ...
      return store.ready(function() {
        ...
        return User.find(1)
          ...
          .exec(function(user) {
            ...
            return user.posts
              .create({ thread_id: 1, message: 'another post' })
              .then(function(post) {
      ...
      return store.ready(function() {
        ...
        return User.find(2)
          ...
          .then(function(user) {
            ...
            user.posts.add(
              Post.new({ thread_id: 1, message: 'yet another post' })
            )
      ...
      return store.ready(function() {
        ...
        return User.find(2)
          ...
          .then(function(user) {
            ...
            user.posts.add({
              thread_id: 1,
              message: 'yet another post without new'
            })
    ...
    it('set a belongsTo record with = record', function() {
      return store.ready(function() {
        ...
        return Thread.find(1).exec(function(thread) {
          thread.user = User.new({
            login: 'new_user',
            email: 'new_user@mail.com'
          })

          return thread.save().then(function() {
            return User.where({ login: 'new_user' })
              .include('threads')
              .first()
              .exec(function(user) {
    ...
    it('set a belongsTo record with = id', function() {
      return store.ready(function() {
        ...
        return Thread.find(1).exec(function(thread) {
          ...
          return thread.user.then(function(user) {
      ...
      return store.ready(function() {
        ...
        return User.find(1).exec(function(user) {
          user.avatar = Avatar.new({
            url: 'http://better-avatar.com/strong.png'
          })
          return user.save().then(function() {
            return Avatar.where({ url_like: 'better' })
              .include('user')
              .first()
              .exec(function(avatar) {
                avatar.url.should.be.equal(
                  'http://better-avatar.com/strong.png'
                )
            ...
            return user.save().then(function() {
              return User.find(2)
                ...
                .exec(function(michl) {
                  ...
                  user.unread[0].attributes.message.should.be.equal(
                    'unread message'
                  )
                })
            })
