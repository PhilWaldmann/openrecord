...
global.testActiveDirectory = function(name, objects) {
  if (
    ...
    console.log(
      'Specify AD_URL=ldap://your.domain AD_USER=DOMAIN\\Administrator AD_PASSWORD=mysecret AD_BASE=dc=your,dc=domain'
    )
    return
  }
...
function flattenTestData(objects, parentDn) {
  var tmp = []
  ...
  for (var i = 0; i < objects.length; i++) {
    ...
    if (
      !obj.dn
        .toLowerCase()
        .replace(' ', '')
        .match(LDAP_BASE.toLowerCase().replace(' ', ''))
    ) {
...
function deleteAll(baseDn, callback) {
  client.search(
    baseDn,
    { scope: 'sub', attributes: ['dn'], filter: '(objectClass=*)' },
    [],
    function(err, res) {
      if (err) throw err
      var results = []
          ...
          ;(function(result) {
            calls.push(function(next) {
              client.del(result, function(err) {
                if (err)
                  throw new Error('unable to destroy ' + result + ': ' + err)
                next()
              })
            })
          })(results[i])
