...
module.exports = function(title, beforeFn, afterFn, storeConf){
  describe(title + ': Exec (' + storeConf.url + ')', function(){
    ...
    it('get all ou objects of the base ou incl. child objects', function(){
      return store.ready(function(){
        var Ou = store.Model('OrganizationalUnit')
        return Ou.searchRoot('ou=openrecord,' + LDAP_BASE, true).exec(function(ous){
    ...
    it('get all ou objects of the base ou inkl. child objects (recursive)', function(){
      return store.ready(function(){
        var Ou = store.Model('OrganizationalUnit')
        return Ou.searchRoot('ou=openrecord,' + LDAP_BASE).recursive().exec(function(ous){
          ous.length.should.be.equal(6)
          ous[0].name.should.be.equal('openrecord')
        })
      })
    ...
    it('get all user objects of base ou', function(){
      return store.ready(function(){
        ...
        return User.searchRoot('ou=openrecord,' + LDAP_BASE).exec(function(users){
    ...
    it('get all user objects of base ou incl. child objects', function(){
      return store.ready(function(){
        var User = store.Model('User')
        return User.searchRoot('ou=openrecord,' + LDAP_BASE, true).exec(function(users){
    ...
    it('get all user objects of one ou', function(){
      return store.ready(function(){
        var User = store.Model('User')
        return User.searchRoot('ou=sub_ou1,ou=exec_test,ou=openrecord,' + LDAP_BASE).exec(function(users){
    ...
    it('returns null on wrong searchRoot', function(){ // TODO: this should instead throw an error - right?
      return store.ready(function(){
        var User = store.Model('User')
        return User.searchRoot('ou=unknown,ou=openrecord,' + LDAP_BASE).exec(function(users){
    ...
    it('user object has standard attributes', function(){
      return store.ready(function(){
        var User = store.Model('User')
        return User.searchRoot('ou=sub_ou3,ou=exec_test,ou=openrecord,' + LDAP_BASE).exec(function(users){
          var user = users[0]
          ...
          user.objectSid.should.match(/S-\d-\d-\d{2}-\d{10}-\d{8,10}-\d{8,10}-\d{3,6}/)
    ...
    it('do a find on a not existing user object', function(){
      return store.ready(function(){
        var User = store.Model('User')
        return User.find('cn=unknown,ou=exec_test,ou=openrecord,' + LDAP_BASE).exec(function(user){
