...
describe('Utils', function(){
  ...
  describe('compareObjects', function(){
    ...
    it('compare objects', function(){
      ...
      utils.compareObjects({foo: 'a'}, {foo: 'a', bar: 'a'}).should.be.equal(false)
      utils.compareObjects({foo: 'a'}, '{"foo": "a"}').should.be.equal(false)
    })

    it('compare deep objects', function(){
      utils.compareObjects({foo: {x: 'a'}}, {foo: {x: 'a'}}, true).should.be.equal(true)
      utils.compareObjects({foo: {x: 'a'}}, true, true).should.be.equal(false)
      utils.compareObjects({foo: {x: 'a'}}, {foo: 'b'}, true).should.be.equal(false)
      utils.compareObjects({foo: {x: 'a'}}, {foo: {x: 'a'}, bar: {x: 'a'}}, true).should.be.equal(false)
    })
  })
  ...
  describe('removedArrayValues', function(){
    it('returns array with removed elements', function(){
      ...
      utils.removedArrayValues(['a', 'b', 'd', 'e'], ['a', 'b', 'c']).should.be.eql(['d', 'e'])
    })
