...
exports.definition = {
  ...
  sortedList: function(config){
    ...
    if(config.scope && !Array.isArray(config.scope)) config.scope = [config.scope]
    ...
    this.beforeSave(function(record, options){
      ...
      if(record[position] !== null){
        ...
        return self.model.useTransaction(options.transaction).max(position).where(condition).exec(function(result){
