  ...
  nestedSet: function(){
    ...
    this.beforeCreate(function(record, options){
      if(record.parent_id){
        ...
        return self.model.find(record.parent_id).transaction(options.transaction).exec(function(parent){
          if(parent){
        ...
        // new root node!
        return self.model.rootOnly().order('rgt', true).limit(1).transaction(options.transaction).exec(function(rootSibling){
          if(rootSibling){
    ...
    this.afterDestroy(function(record, options){
      ...
      var rgtCol = record.definition.store.connection.client.wrapIdentifier('rgt')
      var lftCol = record.definition.store.connection.client.wrapIdentifier('lft')
      return Model.transaction(options.transaction).where({lft_between: [record.lft, record.rgt]}).delete(options)
      .then(function(){
