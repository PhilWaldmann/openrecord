  ...
  nestedSet: function(){
          ...
          return this.model.where({lft_between: ranges}).where(depthConditions).order('lft').exec(function(children){
            for(var i = 0; i < children.length; i++){
              ...
              if(rangeRecords[child.lft - 1]){
                ...
                if(parent){                  
                  parent.relations.children = parent.relations.children || parent.definition.relations.children.collection(parent) // quickfix!
    ...
    this.beforeCreate(function(record, options){
      if(record.parent_id){
        ...
        return self.model.find(record.parent_id).useTransaction(options.transaction).exec(function(parent){
          if(parent){
            return self.query(options).where('rgt', '>=', parent.rgt).increment('rgt', 2).then(function(){
              return self.query(options).where('lft', '>', parent.rgt).increment('lft', 2)
            }).then(function(){
        ...
        // new root node!
        return self.model.rootOnly().order('rgt', true).limit(1).useTransaction(options.transaction).exec(function(rootSibling){
          if(rootSibling){
        ...
        if(record.parent_id){
          return self.model.find(record.parent_id).useTransaction(options.transaction).exec(function(parent){
            if(parent){
              ...
              throw new Error("can't find parent node with id " + record.parent_id)
            }
          })
        }else{
    ...
    this.afterDestroy(function(record, options){
      ...
      var rgtCol = record.definition.store.connection.client.wrapIdentifier('rgt')
      var lftCol = record.definition.store.connection.client.wrapIdentifier('lft')
      return Model.useTransaction(options.transaction).where({lft_between: [record.lft, record.rgt]}).delete(options)
      .then(function(){
        return self.query(options).where('rgt', '>', record.rgt).update({rgt: raw(rgtCol + ' - ' + width)})
      })
      .then(function(){
        return self.query(options).where('lft', '>', record.rgt).update({lft: raw(lftCol + ' - ' + width)})
      })
    })
