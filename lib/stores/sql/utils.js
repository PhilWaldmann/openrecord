exports.utils = {
  ...
  toJoinsList: function(joins){
    ...
    if(joins.length === 1 && Array.isArray(joins[0])) return this.toJoinsList(joins[0])
  ...
  getAttributeColumns: function(definition, joinMapping, parentRelations, isRoot){
    const self = this
    const columns = []
  ...
  hydrateJoinResult: function(data, columns, mapping){ 
    ...
    const self = this
    const records = []
    const columnNames = Object.keys(columns).filter(function(c){ return c !== '$primary' })
    const primaryColumns = columns.$primary || [] // $primary === the primary keys for grouping records
    const baseColumns = columnNames.filter(function(c){ return typeof columns[c] === 'string' })
    const relationColumns = columnNames.filter(function(c){ return typeof columns[c] !== 'string' })
    ...
    data.forEach(function(item){
      ...
      const keyParts = primaryColumns.map(function(c){ return item[columns[c]] }).filter(function(v){ return v !== null })
      ...
      relationColumns.forEach(function(name){        
        const childRecords = self.hydrateJoinResult([item], columns[name], mapping[name]) 
