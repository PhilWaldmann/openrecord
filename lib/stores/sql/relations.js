exports.definition = {
  relation: function(name, options){
    ...
    options.sqlJoin = options.sqlJoin || function(query, join, joinMapping, parentRelations){
      ...
      .then(function(){
        const sql = xquery.toString().replace(/select \* from .x.( where |)/i, '')
      ...
      .then(function(){
        if(!joinMapping.customSelect){ 
          ...
          joinMapping.select = joinMapping.select.concat(utils.getAttributeColumns(targetDefinition, joinMapping, parentRelations))
        }
