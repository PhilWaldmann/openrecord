        ...
        if(relation.type == 'belongs_to' && !relation.through){
          if(record[relation.name] && record[relation.name].hasChanges()){
            ...
            (function(relation){
              ...
              if(relation.validates instanceof Array) validates = relation.validates.indexOf(record[relation.name].__exists ? 'update' : 'destroy') != -1;
              
              tmp.push(function(done){
                record[relation.name].save({transaction:transaction}, function(okay){
                  if(okay || !validates){
                    if(!record.hasChanged(relation.primary_key)){ //check if the relation id was manually changed
            ...
            for(var i = 0; i < this[relation.name].length; i++){
              if(this[relation.name][i] && this[relation.name][i].hasChanges()){
                (function(relation, subrecord){
                  var validates = true;          
                  if(relation.validates === false) validates = false;
                  if(relation.validates instanceof Array) validates = relation.validates.indexOf(subrecord.__exists ? 'update' : 'destroy') != -1;
        ...
        if(relation.type == 'has_one' && !relation.through){
          if(this[relation.name] && this[relation.name].hasChanges()){
            (function(relation){
              var validates = true;          
              if(relation.validates === false) validates = false;
              if(relation.validates instanceof Array) validates = relation.validates.indexOf(this_record[relation.name].__exists ? 'update' : 'destroy') != -1;
    ...
    this.afterDestroy(function(record, transaction, next){
      ...
      for(var i in self.relations){
        ...
        (function(relation){
          var validates = true;          
          if(relation.validates === false) validates = false;
          if(relation.validates instanceof Array) validates = relation.validates.indexOf('destroy') != -1;
