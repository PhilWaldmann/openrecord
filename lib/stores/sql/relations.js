      ...
      for(var i in self.relations){
        ...
        if(relation.type == 'has_one'){
          if(this[relation.name]){
            tmp.push(function(done){
              this[relation.name][relation.foreign_key] = record[relation.primary_key];
              this[relation.name].save({transaction:transaction}, function(okay){                    
                if(okay){
                  return done();
                }
                done('STOP');
          ...
          if(relation.dependent == 'nullify'){
            tmp.push(function(done){
              if(relation.type == 'has_many'){
                ...
                record[relation.name].update_all(attrs, {transaction: transaction, rollback:false, commit:false}, function(success){
                  if(!success) return done('STOP');
                  done();
                });
              }              
