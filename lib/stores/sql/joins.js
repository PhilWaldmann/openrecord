...
exports.model = {
  ...
  join: function(relations, type){
    ...
    if(type && ['left', 'inner', 'outer', 'right'].indexOf(type.toLowerCase()) !== -1){
      args = relations
    }else{
    ...
    joins.forEach(function(join){
      ...
      if(relation){        
        ...
        if(relation.type === 'belongs_to_polymorphic') throw new Error('Can\'t join polymorphic relations')
      ...
      throw new Error('Can\'t find relation "' + join.relation + '" for ' + self.definition.modelName)
    })
...
exports.definition = {
  ...
  mixinCallback: function(){
    ...
    this.beforeFind(function(query){
      ...
      joins.forEach(function(join){
        ...
        if(join.type === 'raw'){          
          ...
          if(typeof relation.sqlJoin === 'function'){
            jobs.push(relation.sqlJoin.call(collection, query, join, mapping, parentRelations))
          }else{
      ...
      return this.store.utils.parallel(jobs)
      .then(function(){
        if(isRootQuery){
