  ...
  mixinCallback: function(){
    ...
    this.beforeFind(function(query){
      ...
      for(var i = 0; i < joins.length; i++){
        ...
        var extra_conditions = Utils.sanitizeConditions(self.model, relation.conditions);
        ...
        query[joins[i].type + 'Join'](table_name + as, function(){
          ...
          for(var i = 0; i < extra_conditions.length; i++){
            ...
            var tname = Utils.nameTreeToNames(cond.model.definition.table_name, cond.name_tree);
            var value = cond.value;

            if(cond.value){ //TODO: check for column names...
              value = self.store.connection.raw('?', cond.value);
            }else{
              value = self.store.connection.raw('NULL');
            }
            
            //TODO: put the primary_key and foreign_key into the conditions hash of the relation
            //this.andOn(tname + '.' + cond.field, cond.operator, value);
            this.andOn(tname + '.' + cond.attribute, '=', value);
          }
        });
      }
