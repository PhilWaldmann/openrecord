      ...
      for(i = 0; i < joins.length; i++){
        ...
        (function(conditions, xquery, join, tableName, as){
          calls.push(function(){
            ...
            return _self._applyCondtions(Utils.reverseConditions(conditions), xquery)
            .then(function(){
              var sql = xquery.toString().replace(/select \* from .x. where /i, '')
              // now put the raw condition query into the join...
              return query[join.type + 'Join'](tableName + as, self.store.connection.raw(sql))
            })
          })
