...
exports.definition = {
  mixinCallback: function(){
    ...
    this.attribute('dn', 'dn', {emit_events: true, primary: true}) // Well... it's not a realy primary key, but it's used internaly for relations...
    ...
    this.on('dn_changed', function(record, oldValue, value){
      if(value){
        record.attributes.parent_dn = self.store.utils.normalizeDn(parseDN(value).parent().toString()) // sets the parent_dn, but we don't want to fire a change event
      }
    })
      ...
      if(this.models[i].definition){
        for(var name in this.models[i].definition.attributes){
          if(this.models[i].definition.attributes.hasOwnProperty(name)){
            if(!binaryOnly || (binaryOnly && this.models[i].definition.attributes[name].type.binary)){
              tmp.push(name)
            }
          }
        }
