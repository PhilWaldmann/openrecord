...
exports.definition = {
  mixinCallback: function(){
    this.use(function(next){
      ...
      this.store.connection.raw('SELECT * FROM user_tab_cols WHERE table_name = \'' + this.table_name + '\'').asCallback(function(err, response){
        console.log('ORACLE ATTRS', err, response)
        if(err){
          next()
        ...
        for(var i in result){
          self.attribute(result[i].Field, simplifiedType(result[i].Type), {
            persistent: true,
