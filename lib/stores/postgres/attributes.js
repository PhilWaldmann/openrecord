...
exports.definition = {
  setTableAttributes(attributes){
    ...
    attributes.forEach(function(attr){
      ...
      if(attr.type === 'composite'){
        ...
        self.createCompositeType(attr.type_name, attr.type_attributes, attr)

        // add custom validation to call composite type validation
        self.validates(function(done){
          var parent = this
          if(this[attr.name] && typeof this[attr.name].isValid === 'function'){
            this[attr.name].isValid(function(valid){
              if(!valid){
  ...
  createCompositeType(name, attributes, attr){
    var store = this.store
    if(store.dynamic_attribute_types[name]){
      var Model = store.dynamic_attribute_types[name]

      attr.options.dynamicType = Model
      attr.options.use = Model.definition.use.bind(Model.definition) // TODO: is currently shared between all attributes with the same type
    ...
    definition.define(function(Model){
      store.dynamic_attribute_types[name] = Model
      attr.options.dynamicType = Model
      attr.options.use = Model.definition.use.bind(Model.definition)
      attr.options.attributes = attributes.map(function(a){
        return a.name
      })
    })
      ...
      return Promise.all(
        result.map(function(attribute){
          ...
          if(!type){
            ...
            return self.loadTableAttributes(typeName)
