...
function addInclude(query, context, info, sub){
  if(query){
    if(query.model && query.include && (!query.getInternal || !query._isResolved())) {
      return query
  ...
  nodes.forEach(function(node) {
    if(node.kind === 'FragmentSpread'){
      ...
      if(fragment.selectionSet){
        Object.assign(includes, toIncludeHelper(Model, fragment.selectionSet.selections, info))
      }
      return
    }
    ...
    if(node.kind === 'InlineFragment'){
      ...
      if(node.selectionSet){
        Object.assign(includes, toIncludeHelper(Model, node.selectionSet.selections, info))
      }
      return
    }
    ...
    if (relation) {
      ...
      if (node.selectionSet){
        if(relation.type === 'belongs_to_polymorphic'){
          includes[relation.name] = toIncludeHelper(Model, node.selectionSet.selections, info)
        }else{
          includes[relation.name] = toIncludeHelper(relation.model, node.selectionSet.selections, info)
        }
      }        
