  ...
  mixinCallback: function(){
      ...
      if(options.through){
        ...
        for(var base in attrs){
          if(attrs[base] && attrs[base].attribute){
            if(options.type == 'has_many' || options.type == 'has_one'){
              if(parent._exists && parent[attrs[base].attribute]){
        ...
        //.... parent_dn will be set, but the set method wont process is, because it's not an attribute...
        if(options.type == 'has_many' || options.type == 'has_one'){
          record.set(attrs);
        }else{
          parent.set(attrs);
    ...
    this.on('relation_initialized', function(record, options, collection){  
      if(options.type == 'has_many' && !options.polymorph){
        var conditions = Utils.clone(options.conditions) || {};

        for(var base in conditions){
          if(conditions[base] && conditions[base].attribute){
            conditions[base] = record[conditions[base].attribute];
          }
        }
