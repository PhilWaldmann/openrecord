...
exports.definition = {
  mixinCallback: function(){
    ...
    this.on('relation_added', function(options){
      var primaryKey = this.primaryKeys[0] || 'id' // TODO: support multiple primary keys...
      ...
      if(options.autoSave === undefined){
        options.autoSave = self.store.config.autoSave || false
      }
      ...
      if(options.type === 'belongs_to'){
        ...
        if(!self.attributes[options.primary_key] && !self.definedGetter[options.primary_key]){ // if there is no primary_key field available, try relation_name + _id
      ...
      if(options.type === 'belongs_to_many'){
        ...
        if(!self.attributes[options.primary_key] && !self.definedGetter[options.primary_key]){ // if there is no primary_key field available, try relation_name + _ids
      ...
      if(!self.attributes[options.primary_key] && !self.definedGetter[options.primary_key]){
      ...
      if(options.type === 'has_many' && options.primary_key){
        ...
        self.attribute(attrName, Array, {
          ...
          setter: function(value){
            this[options.name].lazyDestroyRecords()

            if(value instanceof Array){
              var record = this
              value.forEach(function(id){
                var tmp = {}
                tmp[options.foreign_key] = id
                record[options.name].add(tmp)
              })
            }
      ...
      for(var name in this.definition.relations){
        if(this.definition.relations.hasOwnProperty(name)){
          ...
          if(data){
            ...
            var primaryKeys = relation.model.definition.primaryKeys
