  ...
  mixinCallback: function(){
    ...
    this.on('relation_added', function(options){
      ...
      if(options.primary_key){
        options.conditions = options.conditions || {};
        options.conditions[options.primary_key] = {attribute: options.foreign_key, relation: options.name};
      }
    ...
    this.on('relation_record_added', function(parent, options, record){
      ...
      if(options.conditions[options.name]){
        record.set(options.conditions[options.name])
      }
    ...
    this.on('relation_initialized', function(record, options, collection){  
      if(options.type == 'has_many' && !options.polymorph){
        var conditions = Utils.clone(options.conditions[options.name]) || {};
