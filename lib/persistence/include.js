...
exports.definition = {
  mixinCallback: function(){
    ...
    this.beforeInclude(function(Chain, records, include, cache){
      ...
      var conditions = Utils.sanitizeConditions(Chain.model, conditions);
      var record_map = cache.record_map = cache.record_map || {};
      var condition = {};
      
      for(var i = 0; i < conditions.length; i++){
        var cond = conditions[i];
        if(cond.operator == 'eq' && cond.value && cond.value.attribute){
          var primary_key = cond.attribute;
          var foreign_key = cond.value.attribute;
          
          cache.key = foreign_key;
          condition[foreign_key] = [];
          
          for(var r = 0; r < records.length; r++){
            var id = records[r][primary_key];
            if(id && (!relation.polymorph || (relation.polymorph && records[r][relation.type_key] == Chain.definition.getName()))){
            
              if(condition[foreign_key].indexOf(id) === -1){
                condition[foreign_key].push(id);
              }
    ...
    this.afterInclude(function(result, records, include, cache, Model){
      ...
      if(!include.scope){        
        ...
        for(var i = 0; i < result.length; i++){
          var key = result[i][cache.key]; //TODO: rewrite
          var records = [];
