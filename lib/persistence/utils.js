          ...
          for(var name in relations[i]){
            ...
            if(relation){
              if(relation.through){
                ...
                var rel = exports.utils.sanitizeRelations(parent, throughRel, nameTree, through)
                var originalRel = rel[rel.length - 1]
                originalRel.relation.scope = relation.scope // + add original scope
                originalRel.args = args // add original arguments
              ...
              // just a scope on the current relation (e.g. include(':totalCount'))
              if(scope){
                tmp.push({
                  name_tree: nameTree.concat(parent.definition.getName()),
                  scope: scope,
