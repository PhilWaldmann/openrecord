...
exports.utils = {
  ...
  sanitizeRelations: function(parent, relations, name_tree, through){
    ...
    if(!through) through = {};
    if(!util.isArray(relations)) relations = [relations];
            ...
            if(relation){
              if(relation.through){
                ...
                if(relation.polymorph){
                  ...
                  if(util.isArray(sub_relations) && sub_relations.length == 1) sub_relations = sub_relations[0];
                }
  ...
  sanitizeConditions: function(parent, conditions, name_tree, relation){
    ...
    if(!name_tree) name_tree = [];
    if(!util.isArray(conditions)) conditions = [conditions];
    ...
    for(var i = 0; i < conditions.length; i++){
      ...
      if(typeof conditions[i] == 'string' && i === 0){
        ...
        if(typeof args[0] == 'object' && !util.isArray(args[0])){
          ...
          if(conditions[i].hasOwnProperty(name)){
            ...
            if(current_relation){
              ...
              if(parent.definition.attributes[name]){
                ...
                //ignore the condition if the attribute's type does not have any operators
                if(!attrs[tmp_name]){
                  if(process.env.NODE_ENV !== 'test') parent.definition.store.logger.warn("Can't find any operator for attribute '" + tmp_name + "'");
                  continue;
                }
                ...
                if(!attrs[tmp_name].type.operators[operator]){
                  if(process.env.NODE_ENV !== 'test') parent.definition.store.logger.warn("Can't find operator '" + operator + "' for attribute '" + tmp_name + "' (" + attrs[tmp_name].type.name + ")");
                  continue;
                }
