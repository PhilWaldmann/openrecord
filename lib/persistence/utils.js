          ...
          if(conditions[i].hasOwnProperty(name)){
            ...
            if(current_relation){
              ...
              if(value && typeof value === 'object' && value.attribute){
                var given_name_tree = value.name_tree != null
                value.model = value.model || parent;
                value.name_tree = value.name_tree || name_tree;
                
                if(value.relation && !given_name_tree){                  
                  //search down...
                  var pos = name_tree.slice(0, -1).lastIndexOf(value.relation);
                  if(pos != -1){
                    value.name_tree = name_tree.slice(0, pos + 1);
                  }else{
                    //search up
                    var rel = parent.definition.relations[value.relation];
                    if(rel && value.name_tree.indexOf(value.relation) === -1){
                      value.name_tree = value.name_tree.concat(rel.name);
                      value.model = rel.model;
                    }
                  }                  
                }
              }
              ...
              if(parent.definition.attributes[name]){
                ...
                if(!operator){ 
                  parent.definition.store.logger.warn("Can't find attribute '" + name + "'");
                  continue;
                }
