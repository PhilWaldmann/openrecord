...
exports.definition = {
  ...
  relation: function(name, options){
    ...
    self.getter('_' + name, options.rawGetter || function(){
    ...
    if(options.setter) self.setter(name, function(value){      
      options.init()
    ...
    options.loadWithCollection = options.loadWithCollection || function(collection){
      const parentRecord = collection.getInternal('relation_to')
      const noCache = collection.getInternal('no_relation_cache')
      const existingConditions = collection.getInternal('conditions')
      
      if(!parentRecord) return

      // do we use a cache?
      if(!noCache){
        // does the relatoin support bulk loading
        if(typeof options.getFromCache === 'function'){        
          // return from the cache!
          return options.getFromCache(parentRecord, null, existingConditions) // creates a new collection...
      ...
      const conditions = self.store.utils.recordsToConditions([parentRecord], options.from, options.to)
    ...
    options.loadFromRecord = options.loadFromRecord || function(parentRecord, include){
      if(options.model){
        const conditions = self.store.utils.recordsToConditions([parentRecord], options.from, options.to)
      ...
      throw new Error('You need to implement your own `loadFromRecord` function! (relation: ' + name + ')')
    }
    ...
    options.preInitialize = options.preInitialize || function(){ return this.callParent() }
    options.preInitialize._parent = function(){
      var store = self.store
      if(options.store) store = Store.getStoreByName(options.store)
      if(!store) throw new Error("Unknown store '" + options.store + "' on relation '" + name + "'")
      
      const model = store.Model(options.model || name)
      if(!model) throw new Error("Unknown model '" + options.model + "' on relation '" + name + "'")
      options.model = model
    }
    ...
    options.initialize = options.initialize || function(){ return this.callParent() }
    options.initialize._parent = function(){
      ...
      if(options.from.length !== options.to.length){        
        throw new Error("Can't connect " + self.getName() + "(" + options.from.join(', ') + ") with " + options.model.definition.getName() + "(" + options.to.join(', ') + ")")
      }
    }
  ...
  set: function(field, value){
    ...
    if(typeof field === 'object'){
      ...
      Object.keys(this.definition.relations).forEach(function(relationName){
        if(field[relationName] !== undefined) this[relationName] = field[relationName]
      }, this)
    }
