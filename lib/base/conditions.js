...
exports.model = {
  ...
  where: function(){
    ...
    const isResolved = self._isResolved()

    if(isResolved){ // if the collection is already resolved, return a unresolved and empty copy!
      self = self._unresolve()
    }
    ...
    existingConditions.forEach(function(cond){
      existingMap[cond.type + '|' + cond.attribute + '|' + cond.operator + '|' + JSON.stringify(cond.value)] = true
    })
    ...
    conditions.forEach(function(cond){
      ...
      if(existingMap[cond.type + '|' + cond.attribute + '|' + cond.operator + '|' + JSON.stringify(cond.value)]) return
...
exports.definition = {
  mixinCallback: function(){
    ...
    this.onHashCondition(function(chain, condition, query){
      ...
      if(!attribute) throw new Error("Unknown attribute '" + condition.attribute + "' on model '" + chain.definition.modelName + "'")
