...
exports.store = {
  mixinCallback: function(){
    ...
    Store.addExceptionType(ValidationError)
  }
}
...
var ValidationError = function ValidationError(record){
  Object.defineProperty(this, 'has', {enumerable: false, writable: true, value: false})
  Object.defineProperty(this, 'record', {enumerable: false, writable: true, value: record})
  Object.defineProperty(this, 'errors', {enumerable: true, writable: true, value: {}})
}
...
ValidationError.prototype.toString = function(){
  ...
  Object.keys(self.errors).forEach(function(name){
    ...
    if(error instanceof Array && typeof error[0] === 'string') error = error.join(', ')
    else error = JSON.stringify(error) // TODO: make it prettier!
...
exports.definition = {
  mixinCallback: function(){
    ...
    this.on('relation_record_added', function(parent, options, record){
      ...
      parent.errors = parent.errors || new ValidationError(parent)
      record.errors = record.errors || new ValidationError(record)

      if(options.type === 'has_many' || options.type === 'belongs_to_many'){
        parent.errors.errors[options.name] = parent.errors.errors[options.name] || []
        parent.errors.errors[options.name].push(record.errors.errors)
      }else{
        parent.errors.errors[options.name] = record.errors.errors
      }
    })
  }
}
