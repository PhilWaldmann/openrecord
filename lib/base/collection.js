...
exports.model = {
  ...
  create: function(data){
    ...
    if(Array.isArray(data)){
      return this.chain().add(data).save()
    }
    return this.runScopes()
    .then(function(){
      return self.new(data).save()
    })
  },
...
exports.chain = {
  ...
  add: function(records){
    ...
    records.forEach(function(record){
      ...
      if(record){
        ...
        if(typeof record !== 'object' || (Array.isArray(record) && record.length === primaryKeys.length && typeof record[0] !== 'object')){
          // not a record/object... so it must be a primary key!
          if(self.options.polymorph) throw new Error('Polymorphic relations need to add a record!')
        ...
        if(self.options.polymorph){
          if(record.model && !(record instanceof record.model)) return // only real records are allowed. plain object will be ignored. TODO: Throw an error?!
        }else{
        ...
        if(fromKeys){
          ...
          if(mapKey.length === primaryKeys.length){            
            record._exists()
        ...
        if(relation && parentRecord){
          if(record && relation && typeof relation.add === 'function'){
  ...
  remove: function(index){
    ...
    if(record && relation && parentRecord && typeof relation.remove === 'function'){
...
exports.record = {
  mixinCallback: function(config){
    ...
    Object.defineProperty(this, '__chainedModel', {enumerable: false, writable: true, value: chainedModel})
    Object.defineProperty(this, '__exists', {enumerable: false, writable: true, value: false})
    Object.defineProperty(this, '__lazyOperations', {enumerable: false, writable: true, value: []})
  },
  ...
  _exists: function(){
    ...
    for(var name in this.definition.relations){
      if(this.definition.relations.hasOwnProperty(name)){
        if(this.relations && this.relations[name] && typeof this.relations[name] === 'function'){          
