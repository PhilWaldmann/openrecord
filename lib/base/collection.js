    ...
    for(var i = 0; i < records.length; i++){
      ...
      if(typeof record != 'object'){
        ...
        if(through_rel.type == 'has_many'){
          parent_record.relations[relation.through].add(tmp);
        }else{
          parent_record.relations[relation.through] = tmp;
        }
        ...
        if(relation && parent_record){
          self.definition.emit(parent_record, 'relation_record_added', relation, record);
        } 
      }
    }    
