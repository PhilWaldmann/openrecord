...
exports.model = {
  ...
  inspect: function(indent){
    ...
    if(this.chained){
      ...
      if(records.length === 0){
        if(this.getInternal('resolved')) return '<' + this.definition.modelName + ' [empty result]>'
...
exports.record = {
  ...
  inspect: function(indent, nested){
    ...
    Object.keys(this.definition.relations).forEach(function(name){
      ...
      if(records.inspect) relationTxt = records.inspect(indent + 2, (relation.type !== 'has_many' && relation.type !== 'belongs_to_many'))
