...
exports.store = {
  ...
  addType: function(name, cast, options){
    ...
    if(options.extend && this.field_types[options.extend]){
      var extend = this.field_types[options.extend].cast;
      
      for(var name in extend){
        if(extend.hasOwnProperty(name) && !cast[name]){
          cast[name] = extend[name];
        }
      }
    }
