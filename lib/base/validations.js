...
exports.definition = {  
   ...
   * @params {integer} length - The maximum length
   *
   * @return {Definition}
   */
  validatesLengthOf: function(field, length){    
    return this.validates(field, function(){
      var valid = (this[field].length <= length);
   ...
   * @method validatesLengthOf
   * @param {string} field - The field to validate
   * @params {object} options - The options hash
   *
   * @options
   * @params {boolean} allow_null - Skip validation if value is null
   * @params {integer} eq - value need to be equal `eq`
   * @params {integer} gt - value need to be greater than `gt`
   * @params {integer} gte - value need to be greater than or equal `gte`
   * @params {integer} lt - value need to be lower than `lt`
   * @params {integer} lte - value need to be lower than or equal `lte`
   * @params {boolean} even - value need to be even
   * @params {boolean} off - value need to be odd
   *
   * @return {Definition}
   */
  validatesNumericalityOf: function(field, options){    
    return this.validates(field, function(){
      ...
      if(options.eq && options.eq != value) valid = false;
      if(options.gt && options.gt >= value) valid = false;
      if(options.gte && options.gte > value) valid = false;
      if(options.lt && options.lt <= value) valid = false;
      if(options.lte && options.lte < value) valid = false;
      if(options.even && (value % 2) == 1) valid = false;
      if(options.odd && (value % 2) == 0) valid = false;
      
      if(options.allow_null && !value) valid = true;
