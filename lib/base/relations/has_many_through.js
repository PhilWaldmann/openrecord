...
exports.definition = {
  ...
  _hasManyThrough: function(name, options){
    ...
    options.loadWithCollection = function(collection){
      const parent = collection.getInternal('relation_to')
      const chain = parent[options.through[0]]
    ...
    options.add = function(parent, record){
      if(options.through.length > 2) throw new Error("Doesn't not work with a has many through over " + options.through.length + " relations!")
