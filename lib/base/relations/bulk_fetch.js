exports.definition = {
  ...
  _bulkFetch: function(name, options){
    ...
    //      99 => is the id!
    options.loadFromConditions = options.loadFromConditions || function(parentCollection, parentConditions, include){          
      include = include || {}
      const conditions = self.store.utils.parentConditionsToConditions(parentConditions || [], options.from, options.to)
    ...
    options.loadFromRecords = options.loadFromRecords || function(parentCollection, include, existingConditions){
      include = include || {}
            
      const conditions = self.store.utils.recordsToConditions(parentCollection, options.from, options.to)      
      ...
      return defaultFetch(parentCollection, conditions, include, existingConditions)
    ...
    // TODO: put into utils!?
    function defaultFetch(parentCollection, conditions, include, existingConditions){
      ...
      if(existingConditions){
        ...
        query.addInternal('conditions', existingConditions.filter(function(c){
          var match = false
          intConditions.forEach(function(intC){
            if(options.model.store.utils.compareObjects(c, intC, true)) match = true
          })
          return !match
        }))
      }
    ...
    options.filterCache = options.filterCache || function(parentRecord, records){
        ...
        if(records){
          ...
          const relatedRecords = records.filter(function(record){
            ...
            options.from.forEach(function(key, index){
              ...
              if(!self.store.utils.checkFieldEquality(val1, val2)) match = false
            })
            return match
          })
      ...
      throw new Error('You need to implement your own `filter` function!')
    }
    ...
    options.getFromCache = function(parentRecord, include, existingConditions){
      const parentCollection = parentRecord.__chainedModel || parentRecord.model.chain().add(parentRecord)
      
      return options.loadFromRecords(parentCollection, include || {}, existingConditions)
      .then(function(result){        
