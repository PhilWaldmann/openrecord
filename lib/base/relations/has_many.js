  ...
  hasMany: function(name, options){
    ...
    options.initialize = options.initialize || function(){
      ...
      if(targetPrimaryKeys.length === 1){
        ...
        definition.attribute(inflection.singularize(name) + '_' + inflection.pluralize(key), Array, {
          hidden: true,
          setter: function(values){                      
            this[name].add(values) // the same as `record.relation.add([1, 2])`. But with a nicer syntax: `record.relation_ids = [1, 2]`
          },
    ...
    options.setter = options.setter || function(records){      
      ...
      if(!chain){
        if(!options.model) throw new Error('You need to implement your own `setter` function')
      ...
      const targetPrimaryKeys = options.model.definition.primaryKeys || []
      const recordsToRemove = utils.distinctRecords(chain, records, targetPrimaryKeys)
    ...
    options.add = options.add || function(parent, record){
      ...
      options.to.forEach(function(key, index){        
        ...
        if(Array.isArray(currentValue)){          
          if(currentValue.indexOf(parent[opposite]) === -1) currentValue.push(parent[opposite])          
        }else{                    
    ...
    // clear all records
    options.clear = options.clear || function(parent, collection){
      if(!options.model) throw new Error('You need to implement your own `setter` function')
      const targetPrimaryKeys = options.model.definition.primaryKeys || []
      const records = collection.getInternal('__clear_only') || collection
      const conditions = utils.recordsToConditions(records, targetPrimaryKeys, targetPrimaryKeys)
      if(!conditions) return 

      collection.clearInternal('__clear_only')
      
      parent.relations[name]._lazyOperation(function(){
        if(options.dependent === 'delete'){
          return options.model.where(conditions).deleteAll()
        }
        if(options.dependent === 'destroy'){
          return options.model.where(conditions).destroyAll()
        }
        // nullify!        
        const update = {}
        options.to.forEach(function(key){
        ...
        return options.model.where(conditions).updateAll(update)
      })
