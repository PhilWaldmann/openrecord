exports.definition = {
  ...
  belongsToPolymorphic: function(name, options){
    ...
    options.afterDestroy = options.afterDestroy || function(parent, transOptions){
      ...
      const conditions = utils.recordsToConditions([parent], [options.idField], [Model.definition.primaryKeys[0]])
      if(!conditions) return

      var dependent = options.dependent
      if(typeof dependent === 'object') dependent = dependent[parent[options.typeField]] // Allow for `dependent: {'ModelA': 'delete', 'ModelB': 'nullify'}`
    ...
    options.loadFromRecords = function(parentCollection, include){
      ...
      Object.keys(targetMap).forEach(function(key){
        ...
        if(!model) throw new Error("Unknown model '" + keyParts[1] + "' on polymorphic relation '" + name + "'")
