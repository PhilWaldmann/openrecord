...
exports.definition = {
  belongsToMany: function(name, options){
    ...
    options.preInitialize = options.preInitialize || function(){
      ...
      options.from = options.from || targetPrimaryKeys.map(function(key){
        var fromKey = inflection.singularize(options.model.definition.getName().toLowerCase()) + '_' + key + 's'  
        if(!definition.attributes[fromKey]) fromKey = name + '_' + key + 's'
        return fromKey
      })
    ...
    options.initialize = options.initialize || function(){
      ...
      options.from.forEach(function(attributeName){
        ...
        definition.attributes[attributeName].notifications.push(function(values){
    ...
    options.add = options.add || function(parent, record){
      ...
      options.to.forEach(function(key, index){
        ...
        ids.push(record[key])
        parent.attributes[opposite] = utils.uniq(ids)           
      })      
    ...
    options.beforeSave = options.beforeSave || function(parent, saveOptions){
    ...
    options.afterDestroy = options.afterDestroy || function(parent, transOptions){
    ...
    options.clear = options.clear || function(parent, collection){
      ...
      parent.relations[name]._lazyOperation(function(transOptions){
        ...
        options.from.forEach(function(key, index){
          parent[key] = collection.map(function(record){            
